...
面向 vibe coding 开发者的设计助手。将**网站链接**、截图、情绪图片、非 UI 灵感、音乐和直觉感受转化为结构化的设计系统、动效语言、C 端 App UIUX 系统、视觉素材方向与符合产品语境的空间布局 —— 默认会结合你的产品背景生成 **3 个视觉方向**，再正式整理 Token，让结果先可见、可比较，而不是过早锁定。所有探索都通过独立的预览页面协作完成，只有当你准备好时才会将设计应用到项目。
...
提供**网站 URL**和/或 UI 截图、设计稿。有链接时，Agent 会访问页面、读取前端线索、按需截取局部视觉，并可观察动效 —— 你不必上传十几 MB 的整页截图。该技能将提取其完整的"风格 DNA"并生成一个**独立预览页面**供你评估 —— 在你确认之前不会修改你的项目：
...
1. 说明你的项目用途和目标用户
2. 分享灵感 —— **网站链接**（对线上站点优先）、灵感图片（风景、物体、其他应用），**或音乐录音**（音频片段、哼唱旋律、歌曲片段）
3. 技能将视觉信号与音频信号一同转化为设计特质 —— 节奏、音色和韵律会映射为 UI 中的活力感、温度感和质感。对 URL，Agent 自行浏览页面，而不是先让你压缩巨型截图
...
```
# 用 URL 提取设计系统（Agent 访问页面，无需上传巨型截图）
"分析 https://example.com 的设计，并给我设计 Token"

# 没有 URL 时再用截图提取
"分析这张截图的设计，并给我设计 Token"

# 用线上站点作为灵感探索（生成 3 个概念稿 + 情绪看板）
"以 https://example.com 为灵感 —— 为我的产品给出 3 个视觉方向"
...
│   ├── INSPIRATION-SOURCES.md        # URL 优先接入（浏览、CSS、局部截取、动效观察）
