...
提供**网站 URL**和/或 UI 截图、设计稿 —— 你自行选择素材形式。若提供链接，Agent 可访问页面、读取前端线索、按需截取局部视觉，并观察动效。该技能将提取其完整的"风格 DNA"并生成一个**独立预览页面**供你评估 —— 在你确认之前不会修改你的项目：
...
1. 说明你的项目用途和目标用户
2. 分享灵感 —— **网站链接**、灵感图片（风景、物体、其他应用），**或音乐录音**（音频片段、哼唱旋律、歌曲片段）。形式由你选择；Agent 按你实际提供的素材适配
3. 技能将视觉信号与音频信号一同转化为设计特质 —— 节奏、音色和韵律会映射为 UI 中的活力感、温度感和质感。对 URL 可浏览页面；对截图/图片则直接分析
...
│   ├── INSPIRATION-SOURCES.md        # 灵感素材接入（URL / 图片 / 音乐 — 按用户所提供适配）
