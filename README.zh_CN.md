...
> **设计上下文**：当你开始构建任何 UI 项目时，vibe-to-ui 会自动建立或加载 `DESIGN.md` 中的设计上下文 —— 让每一个设计决策从一开始就服务于产品目标。
...
vibe-to-ui 会在项目根目录读写一个共享的 `DESIGN.md` 文件 —— 跨会话积累产品知识（目标用户、使用场景、产品个性、设计决策）。支持两种模式：

**被动模式**（自动）—— 在任何设计对话中自动发生，无需你主动触发：
- **读取**产品上下文后再做设计决策 —— 让设计服务于真实的产品目标
- **写入**对话中获得的上下文 —— 无需手动记录设计理由
- **协作**其他技能（PM、用户研究等）—— 它们将产品上下文写入 `DESIGN.md`，vibe-to-ui 读取

**主动模式**（按需触发）—— 只需说“更新 DESIGN.md”或“把我们聊的写进 DESIGN.md”：
- 回顾当前对话中的产品上下文
- 创建或更新 `DESIGN.md`
- 展示沉淀内容摘要，供你确认和修正
...
这些能力可自然串联，遵循**探索 → 选择 → 应用**的模式，上下文沉淀在全过程中被动运行：

```
[PM 技能将产品上下文写入 DESIGN.md]
     ↓
探索氛围（基于产品上下文）→ 从 3 个概念稿 + 情绪看板中选择 → 提取设计系统 → 预览 → 应用到项目
     ↓
[DESIGN.md 沉淀设计决策与用户洞察]
```

也可灵活混搭 —— 从一个网站提取风格，应用到另一个网站的布局上。Agent 只在你说"应用"时才会修改你的项目（DESIGN.md 除外，它会被动更新）。
