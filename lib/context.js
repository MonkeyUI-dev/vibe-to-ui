...
function resolveHomeRoot() {
  ...
  if (!home) {
    throw new Error(
      'Cannot resolve home directory. Design Context requires a user home path so profiles can live under ~/.vibe-to-ui.'
    );
  }
...
function permissionHint(dir, err) {
  return (
    `Design Context needs write access to ${dir}\n` +
...
function cmdList(root) {
  // Read-only: never create ~/.vibe-to-ui just to list.
